...
get_filtered_snp_annot <- function(snp_annot_file_name) {
  ...
  # Step 2: Convert data.table to data.frame for dplyr operations
  setDF(snp_annot) 
...
get_maf_filtered_genotype <- function(genotype_file_name,  maf, samples) {
  # gt_df <- read.table(genotype_file_name, header = T, stringsAsFactors = F, row.names = 1, sep = "\t", na.strings = c("", "NA", "."), check.names = FALSE)
  ...
  gt_df$V1 <- NULL
  effect_allele_freqs <- colMeans(gt_df, na.rm = T) / 2
...
main <- function(snp_annot_file, gene_annot_file, genotype_file, expression_file,
                 chrom, prefix, maf=0.01, n_folds=10, n_train_test_folds=5,
