...
get_filtered_snp_annot <- function(snp_annot_file_name) {
  ...
  # Step 3: Filter and process the data using dplyr
  snp_annot <- snp_annot %>%
...
get_fm_penalty <- function(pip_df, cis_gt, gene, pip_threshold=0.00001, filt_cluster=F){
  pip_gene <- pip_df %>% filter(ensg==gene) %>% filter(pip > pip_threshold)
  pip_scores <- pip_gene$pip

  if(filt_cluster==T){
    pip_scores <- pip_scores %>% group_by(cluster) %>% top_n(n=1,wt=pip)
  }else {
    pip_scores<-pip_scores
  }
  ...
  matched_pips <- pip_scores[colnames(cis_gt)]
  matched_pips[is.na(matched_pips)] <- 0

  penalty_factors <- 1 - matched_pips  
  #print(length(penalty_factors))
  #print(ncol(cis_gt))
...
do_covariance <- function(gene_id, cis_gt, varIDs) {
...
main <- function(snp_annot_file, gene_annot_file, genotype_file, expression_file,
                 fm_file, chrom, prefix, maf=0.01, n_folds=10, n_train_test_folds=5,
  ...
  weights_col <- c('gene_id', 'varID', 'ref', 'alt', 'beta')
  ...
  # gene_of_interest <- "ENSG00000223875.1"  # Replace with your gene ID
  # if (gene_of_interest %in% genes) {
  #   i <- which(genes == gene_of_interest)
  #   gene <- genes[i]
  ...
  for (i in 1:n_genes) {
    ...
    penalty <- get_fm_penalty(pip_df, cis_gt, gene)
    print(gene_name)
    print(paste0("Is Penalty Null (no fm data)?", is.null(penalty)))
    if (is.null(penalty))
      print("Creating Penalty Vector with 1s")
    if (is.null(penalty) || length(penalty) != ncol(cis_gt)) {
      penalty <- rep(1, ncol(cis_gt))
    }
    print(paste0("Length of cis_gt: ", ncol(cis_gt), " and Length of penalty vector: ", length(penalty)))
    if (all(is.na(cis_gt))) {
    ...
    if (ncol(cis_gt) >= 2) {
      ...
      rownames(adj_expression) <- rownames(expr_df)  # Ensure row names are set
      #cat("Expression data dimensions (before filtering):", dim(adj_expression), "\n")
      
      adj_expression <- adj_expression[rownames(adj_expression) %in% rownames(cis_gt), , drop = FALSE]
      cat("Expression data dimensions (after filtering):", dim(adj_expression), "\n")
      cat("Genotype data dimensions:", dim(cis_gt), "\n")
      
      if (null_testing)
        adj_expression <- sample(adj_expression)
      perf_measures <- nested_cv_elastic_net_perf(cis_gt, adj_expression, n_samples, n_train_test_folds, n_folds, alpha, samples, penalty)
      ...
      fit <- tryCatch(cv.glmnet(cis_gt, adj_expression, nfolds = n_folds, alpha = 0.5, type.measure='mse', foldid = cv_fold_ids, keep = TRUE,penalty.factor=penalty),
      ...
      if (length(fit) > 0) {
        ...
        if (fit$nzero[best_lam_ind] > 0) {
          ...
          weighted_snps_info <- snp_annot %>% filter(varID %in% weighted_snps) %>% select(varID, ref_vcf, alt_vcf)
