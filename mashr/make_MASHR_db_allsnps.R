...
for (c in codes){
  ...
  for (working_gene in gene_list){
    mashr_in <- read.table(mashr_dir %&% "/" %&% working_gene %&% '_MASHR_beta.txt.gz', header = T) %>% select(gene, snps, snp_ID, contains(c)) #%>% 
  ...
  # Make column with REF and ALT alleles and filter out SNPs with 0 weights
  ...
  # Rename rsid and varID to match PrediXcan format
