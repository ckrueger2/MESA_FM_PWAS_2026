...
# Get gene names in the UDR output files directory
...
for (c in codes){
  ...
  for (working_gene in gene_list){
    ...
    #Read in weights
    udr_in <- read.table(
      udr_dir %&% "/" %&% working_gene %&% '_udr_beta.txt.gz', 
      header = T
    ) %>% select(gene, snp_ID, snps, contains(c)) 
    
    #Read in lfsr data and filter by top 250 snp by lowest lfsr 
    lfsr_file <- file.path(udr_dir %&% "/" %&% working_gene %&% "_udr_lfsr.txt.gz")
    ...
    if (file.exists(lfsr_file)) {
      ...
      # Filter weights against the protein-specific top 250 list of SNPs by lowest lfsr. 
      filtered_gene_weights <- udr_in %>%
  ...
  # Make final db files and text files
